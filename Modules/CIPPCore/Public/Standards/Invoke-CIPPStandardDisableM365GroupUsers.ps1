function Invoke-CIPPStandardDisableM365GroupUsers {
    ...
    ($CurrentState.values | Where-Object { $_.name -eq 'EnableGroupCreation' }).value = 'true'
    ...
    (New-GraphPostRequest -tenantid $tenant -Uri "https://graph.microsoft.com/beta/settings/$($CurrentState.id)" -Type patch -Body $body -ContentType 'application/json')
            Write-LogMessage -API 'Standards' -tenant $tenant -message 'Standards API: Enabled users from creating M365 Groups.' -sev Info
        }
        catch {
            Write-LogMessage -API 'Standards' -tenant $tenant -message "Failed to enable users from creating M365 Groups: $($_.exception.message)" -sev 'Error'
        }
    }
    if ($Settings.alert) {
