function Invoke-CIPPStandardDisableSecurityGroupUsers {
    ...
    If ($Settings.remediate) {
        ...
        try {
            $body = '{"defaultUserRolePermissions":{"allowedToCreateSecurityGroups":true}}'
    (New-GraphPostRequest -tenantid $tenant -Uri 'https://graph.microsoft.com/beta/policies/authorizationPolicy/authorizationPolicy' -Type patch -Body $body -ContentType 'application/json')
            ...
            Write-LogMessage -API 'Standards' -tenant $tenant -message 'Standards API: Enabled users from creating Security Groups.' -sev Info
        }
        catch {
            Write-LogMessage -API 'Standards' -tenant $tenant -message "Failed to enable users from creating Security Groups: $($_.exception.message)" -sev 'Error'
        }
    }
